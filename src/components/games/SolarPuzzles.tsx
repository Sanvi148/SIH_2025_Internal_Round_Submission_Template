...
const SolarPuzzles = ({ onComplete, onBack, userName }: SolarPuzzlesProps) => {
  ...
  }, [panels, sunAngle, weatherCondition]);
  ...
  const calculateTotalEfficiency = () => {
    ...
    panels.forEach(panel => {
      ...
      // Optimal angle is close to sun angle
      ...
      efficiency = angleEfficiency * shadingPenalty * weather.multiplier;
      total += efficiency;
      
      // Update panel efficiency
      setPanels(prev => prev.map(p => 
        p.id === panel.id ? { ...p, efficiency } : p
      ));
    });
                      ...
                      {panels.map((panel) => (
                        ...
                        >
                          ...
                          <div className={`absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-bold ${
                            getEfficiencyColor(panel.efficiency)
                          }`}>
                            {Math.round(panel.efficiency)}%
                          </div>
